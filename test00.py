...
# 검색창을 찾는 것
elem = driver.find_element_by_name("q") # find_element 를 통해 원하는 종류를 선택하여 검색 가능
# 찾는 키워드 입력
...
while True: # 무한반복
    ...
    if new_height == last_height: # 스크롤 다운했을 때 더 나오는 것이 없다면 스크롤 높이가 같을 것 / 내렸을 때 추가 이미지가 있다면 스크롤이 길어진다면 new height > last height
        try:
            driver.find_element_by_css_selector(".abcd").click()
...
for image in images:
    try:
        image.click()
        time.sleep(2)
        imgUrl = driver.find_elements_by_xpath("").get_attribute("src")
